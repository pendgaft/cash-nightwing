 ...
 * the active and purged networks. But in this version, the traffic can
 * only go from the wardens, but cannot go through.
 * 
 * @author Bowen 
 */
...
public class TrafficStat1 {
	...
	/** store the paths of one purged node from different providers to the same destiny in a list */
	...
	/**
	 * add an unit to the given AS,
	 * if the AS is in the HashMap, increase its value by one,
	 * if it isn't in the HashMap, put it in it.
	 * @param tHop		the ASN of the AS
	 */
	 ...
	 * @param path		the path to add traffic unit
	 */
	public void addTrafficUnit(BGPPath path) {
	...
	/**
	 * add the path to the path container which stores all the paths
	 * of a purged AS going to a key AS via different providers
	 * @param key		a destiny AS
	 * @param path		a path to the key AS
	 */
	...
	/**
	 *  the traffic of all the paths starting from the 
	 * given AS in activeMap to all ASes in the activeMap
	 * @param tAS		the AS to start from
	 */
	public void activeToActive(AS tAS) {
		...
		for (int t2ASN : this.activeMap.keySet()) {
			...
			// print out percentatge to check the correctness, logger class, print partial res, write more comments!!
			/* if the path exists, do the statistic */
			addTrafficUnit(tpath);
		}
	}
	
	/**
	 * count the traffic of all the paths starting from the 
	 * given AS in activeMap to all ASes in the purgedMap
	 * To get to the purged ASes, get all the providers of that ASes,
	 * and getPathToPurged will pick a best path.
	 * @param tAS		 the AS to start from
	 */
	public void activeToPurged(AS tAS) {
		...
		for (DecoyAS purgedAS : this.purgedMap.values()) {
			...
			addTrafficUnit(purgedAS.getASN());
			System.out.println(" to " + purgedAS.getASN());//!!
		}
	}
	
	/**
	 * count the traffic of paths starting from activeMap
	 * ending at either activeMap or purgedMap
	 **/
	...
	/**
	 * count the traffic of all the paths starting from every 
	 * AS in purgedMap to all ASes in the activeMap.
	 * 
	 * Get the providers of the purged AS, then get all the 
	 * paths from the providers to other ASes in activeMap,
	 * and temporarily store in the container, then get the best
	 * path from the pathSet, and do the statistic.
	 */
	public void purgedToActive() {
		...
		for (int t1ASN : this.purgedMap.keySet()) {
			...
			for (AS pAS : providers) {
				for (int t2ASN : this.activeMap.keySet()) {
					...
					if (tpath == null) continue;
					 ...
					 /* insert the src which is the provider in the front of the path,
					  * be careful about java here! need to do with a copy or, the old
			...
			for (int tASN : pathSets.keySet()) {
				...
				BGPPath tpath = this.purgedMap.get(t1ASN).pathSelection(pathSets.get(tASN));			
	 ...
	 * AS in purgedMap to all ASes in the activeMap.
	 * 
	 * Get the providers of the purged AS, then get the paths to
	 * the providers of other purged ASes, then get the best path of 
	 * all of those by calling pathSelection(), and do the statistic.
	 */
	...
	/**
	 * count the traffic of paths starting from purgedMap
	 * ending at either activeMap or purgedMap
	 **/
